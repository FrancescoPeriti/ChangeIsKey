...
class Extraction(abc.ABC):
    ...
    def __init__(self, pretrained: str = 'bert-base-uncased', output_hidden_states: bool = False,
        ...
        self.model = AutoModel.from_pretrained(pretrained, output_hidden_states=output_hidden_states,
    ...
    def _random_sampling(self, dataset: Dataset, n: int) -> Dataset:
        """
...
class WordEmbeddingExtraction(Extraction):
    ...
    def extract(self, dataset: str, batch_size:int=8, max_length:int=512, layers:int=12, sampling:int=0, agg_sub_words:str='mean') -> dict:
        """
        ...
        for i in range(0, tokenized_text.shape[0], batch_size):
            ...
            for i, row in enumerate(batch_tokenized_text):
                ...
                for l in range(1, layers + 1):
                    sub_word_state = hidden_states[l, i][n_previous_tokens: n_previous_tokens + n_word_token]
...
class AttentionExtraction(Extraction):
    def __init__(self, pretrained: str = 'bert-base-uncased'):
        super().__init__(pretrained, output_hidden_states=False, output_attentions=True)


    def extract(self, dataset: str, batch_size: int = 8, max_length: int = 512, layers: int = 12,
                sampling: int = 0) -> list:
        """
        Extracts attention weights for each token in the input sentences.

        Args:
            dataset (str): The path to the dataset directory containing the sentences to extract attention weights from.
            batch_size (int, default=8): The batch size to use for extracting the attention weights.
            max_length (int, default=512): The maximum sequence length to use for tokenization.
            layers (int, default=12): The number of layers in the pre-trained model to extract embeddings from attention weights from.
            sampling (int, default=0): The number of examples to sample from the dataset. If set to 0, all examples in the dataset are used.

        Returns:
            A list of attention weights tensors. The i-th tensor corresponds to the attention weights for the i-th example in the input dataset.
            The tensor has shape (num_layers, num_heads, seq_length, seq_length), where num_layers and num_heads are the number of layers and heads in the BERT model respectively, and seq_length is the length of the input sequence.
        """
        dataset = self._load_dataset(dataset)
        ...
        for i in range(0, tokenized_text.shape[0], batch_size):
            ...
            attns = torch.stack(output['attentions']).permute(1, 0, 2, 3, 4)
