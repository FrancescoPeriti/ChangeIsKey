...
declare -a models=("bert-base-multilingual-cased" "KB/bert-base-swedish-cased")

for model in "${models[@]}"
do
   python src/attentions.py -t "${tokenized_dataset}" -m "${model}" -M "${max_length}" -l "${layers}" -b "${batch_size}" -o "${output}" -n "${sampling}" -T "${targets}"
done
