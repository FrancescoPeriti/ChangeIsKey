...
for model in "${models[@]}"
do
    python src/attentions_wic.py -t "${tokenized_dataset}/train" -m "${model}" -o "${output}/train"
    python src/attentions_wic.py -t "${tokenized_dataset}/validation" -m "${model}" -o "${output}/test"
done
